...
void readInput(){
  cin >> matrix_n;
  matrix.resize(matrix_n*matrix_n);

  forn(i, matrix_n * matrix_n) {
    cin >> matrix[i];
  }
}
...
void calcFunction() {
  ...
  // count repeated
  forn(i, matrix_n) {
    flags.assign(matrix_n, false);

    forn(j, matrix_n) {
      const auto x = matrix[i * matrix_n + j];
  ...
  forn(i, matrix_n) {
    ...
    forn(j, matrix_n) {
      const auto x = matrix[j * matrix_n + i];
