...
str make_str(v(ll)& arr) {
  str r{};
  std::transform(arr.cbegin(), arr.cend(), std::back_inserter(r), [](ll xx){return xx == 0 ? '0' : '1';});
  return r;
}
  ...
  while(true) {
    ...
    if (mode == 0) {
      ...
      while (true) {
        ...
        // step 1: find same/diff positions
        ll pa = progress;
        ll pb = nbits - progress - 1;
        str answer;
        ...
        // std::cerr << pa << ": " << arr[pa] << ", " << pb << ": " << arr[pb] << std::endl;
        progress++;
        query_counter += 2;

        if (progress == (nbits / 2)) {
          // done
          std::cout <<make_str(arr) << std::endl;
          str answer;
          std::cin >> answer;
          if (answer == "N") {
            std::exit(0);
      ...
      // query known bits to determine fluctuation
      assert (pos_diff >= 0 || pos_same >= 0);
      if (pos_diff < 0) {
        // special case: all are the same so far
        std::cout << pos_same + 1 << std::endl;
        ll q, qnd;
        std::cin >> q;
        ...
        // special case all are different so far
        std::cout << pos_diff + 1 << std::endl;
        ll q, qnd;
        std::cin >> q;
        ...
        if (same == arr[pos_same]) {
          if (diff == arr[pos_diff]) {
            // do nothing
          }
          else {
            rev(arr);
