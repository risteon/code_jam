...
void readInput(){
  ...
  d.resize(s.length());
  std::transform(s.cbegin(), s.cend(), d.begin(), [](char c){return static_cast<ll>(c - '0');});
}
...
void calcFunction() {
  ...
  for (const auto x : d) {
    diffs.push_back(nested_depth - x);
    length += std::abs(diffs.back());
    nested_depth = x;
  }

  v(char) oo;
  oo.resize(length + 1, 'x');

  auto pos = oo.begin();
  char ch = '(';

  assert(d.size() == diffs.size());

  forn(i, diffs.size()) {
    ch = diffs[i] < 0 ? '(' : ')';
    auto nch = std::abs(diffs[i]);
    std::fill_n(pos, nch, ch);
    pos += nch;
    *pos = '0' + d[i];
    pos += 1;
  }
